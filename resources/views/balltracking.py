...
def runVideo():
    ...
    valueList = [entryAngle,exitAngle,arcHeight]
    return valueList;

#creates a text file with the relevant data

def createFile(dlist):
    entryAngle = dlist[0]
    exitAngle = dlist[1]
    arcHeight = dlist[2]
    f = open('data.txt', 'a')
    f.write(str(exitAngle)+'\n')
    f.write(str(entryAngle)+'\n')
    f.write(str(arcHeight)+'\n')
    f.close()
